...
def fetch_from_cache(hash_no):
    try:
        ...
        if local_ip in h_list:
            ...
            H += 1
            cur.execute(
                "update CacheTable set DateTime = '" + _time + "' where Hash = '" + hash_no + "' and Host_ip = '" + local_ip + "';")
            con.close()
...
def cache_performance():
    ...
    print('----------------------------------------------------------')
    cmd = "echo 'lfu{}_local_hits = {} \nlfu{}_miss = {} \nlfu{}_mec_hit = {} \n" \
          "lfu{}_total_hit = {}' >> /home/mec/cache_result.py".format(cache_size, H, cache_size, M, cache_size, MH,
                                                                      cache_size, H + MH)
    os.system(cmd)
...
def run_me():
    ...
    zipf_param = float(input("Zipf paramter [1.1 - 5], could be float: "))
    for i in range(1, n + 1):
    ...
    while True:
        ...
        if s == 'stop':
            ...
            cmd = "echo 'lfu{}_rtt = {} \nlfu{}_cpu = {}' >> /home/mec/cache_result.py".format(cache_size,
                                                                                               calculate_mov_avg(
                                                                                                   x_axis), cache_size,
                                                                                               calculate_mov_avg(
                                                                                                   y_axis))
