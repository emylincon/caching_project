...
import random
import poly_fit_pluggable as pfit
import time
import psutil
    ...
    try:
        ...
        if r >= cache_size:
            ...
            if hash_time_series_dic[min_list_len] > 4:              # the minimum length of list > 4 is the condition #  for using polynomial of degree 3 for prediction
                h1 = Thread(target=pfit.main, args=(hash_times[hashes[0]], 3))
                h2 = Thread(target=pfit.main, args=(hash_times[hashes[1]], 3))
                h3 = Thread(target=pfit.main, args=(hash_times[hashes[2]], 3))
                # h4 = Thread(target=pfit.main, args=(hash_times[hashes[3]], 3))

                h1.start()
                h2.start()
                h3.start()
                # h4.start()
                dic = {hashes[0]: h1, hashes[1]: h2, hashes[2]: h3}
                # dic = {hashes[0]: h1, hashes[1]: h2, hashes[2]: h3, hashes[3]: h4}
                max_hash = max(dic, key=dic.get)
