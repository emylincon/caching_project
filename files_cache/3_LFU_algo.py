...
def getting_ready():
    ...
    cmd = [f"nmap {router_ip}/24" + " -n -sP | grep report | awk '{print $5}'"]
    hosts = str(sp.check_output(cmd, shell=True), 'utf-8')[0:-1].split('\n')

    mec_set = {my_ip, router_ip}

    available_host = set(hosts) - mec_set
...
def run_me():
    ...
    while True:
        ...
        if s == 'stop':
            ...
            for i in tp:
                kv = i.split(':')
                mec_list[kv[0].strip()] = kv[1].strip()
